#<PropertyList><module>c421</module><script><![CDATA[
...
var KNS80 = props.globals.getNode("/instrumentation/kns80",1);
var NAV1 = props.globals.getNode("/instrumentation/nav[0]/frequencies/selected-mhz",1);
var NAV1_RADIAL = props.globals.getNode("/instrumentation/nav[0]/radials/selected-deg",1);
var KNS80_wpt_freq=[];
...
var DME_mhz=props.globals.getNode("instrumentation/dme[0]/frequencies/selected-mhz",1);
var DME_src=props.globals.getNode("instrumentation/dme[0]/frequencies/source",1);
var FDM_ON = 0;
var dsp_flash = props.globals.getNode("instrumentation/kns80/flash", 1);
aircraft.light.new("instrumentation/kns80/dsp-state", [0.5, 0.5],dsp_flash);
...
# check to see if we are in-range
  ...
  #### Nav Modes  0 = VOR ; 1 = VOR/PAR ; 2 = RNAV/ENR ; 3 = RNAV/APR ;
   ...
   var distance=getprop("/instrumentation/dme[0]/indicated-distance-nm");
...
#  var theta = KNS80.getNode("wpt[" ~ use ~ "]/radial").getValue();
  ...
  if ( needle_deflection >  10) {
    needle_deflection = 10;
  }
  if ( needle_deflection < -10) {
    needle_deflection =-10;
  }
...
setlistener(KNS80_serviceable, func(srv){
  setprop("/instrumentation/nav[0]/serviceable",srv.getValue());
  setprop("/instrumentation/dme[0]/serviceable",srv.getValue());
},1,0);

setlistener(KNS80_volume, func(vol){
  setprop("/instrumentation/nav[0]/volume",vol.getValue());
  setprop("/instrumentation/dme[0]/volume",vol.getValue());
},1,0);
...
setlistener(KNS80_dme_hold, func(hld){
  if ( hld.getBoolValue() ) {
    ...
    DME_src.setValue("/instrumentation/dme[0]/frequencies/selected-mhz");
  } else {
